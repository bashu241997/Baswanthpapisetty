...
const SkillGroup = ({ title, skills, index }) => (
  ...
  >
    <h3 className="text-[15px] font-bold text-slate-900 mb-6 flex items-center gap-2">
      <span className="w-1 h-5 rounded-full bg-gradient-to-b from-[#3b82f6] to-[#0ea5e9]" />
      {title}
    </h3>
    ...
    <div className="flex flex-wrap gap-2.5">
      {skills.map((skill) => (
        <span 
          key={skill} 
          className="px-3.5 py-2 rounded-lg bg-gray-50 border border-gray-200/60 text-[12px] text-slate-700 font-medium hover:text-[#3b82f6] hover:border-[#3b82f6]/30 hover:bg-[#3b82f6]/5 transition-all duration-200 cursor-default"
        >
          {skill}
        </span>
      ))}
