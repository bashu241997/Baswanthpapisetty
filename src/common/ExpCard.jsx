      ...
      <div className="flex items-center gap-3.5">
        <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden" style={{ background: experience.iconBg }}>
          {experience.icon ? (
            ...
            <span className="text-[#4a729f] font-black text-base">{experience.company_name[0]}</span>
          )}
        </div>
        <div>
          <h3 className="text-slate-900 text-[16px] font-bold leading-snug">{experience.title}</h3>
          <p className="text-[#4a729f] font-semibold text-[13px] mt-0.5">{experience.company_name}</p>
        </div>
      </div>
    ...
    <ul className="space-y-3">
      {experience.points.map((point, i) => (
        <li key={i} className="flex items-start gap-2.5 text-[13px] text-slate-600 leading-relaxed">
          <span className="w-1.5 h-1.5 rounded-full bg-[#4a729f] flex-shrink-0 mt-1.5 shadow-sm shadow-[#4a729f]/20" />
          <span>{point}</span>
        </li>
      ))}
    </ul>
